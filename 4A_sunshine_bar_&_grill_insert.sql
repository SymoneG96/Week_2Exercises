...
VALUES
   ...
   SELECT Customer_ID, Name, Email_Address FROM Customers;
   SELECT Transaction_ID, Customer_ID, Vendor_ID, Date_Time, Total_Amount FROM Sale_Transactions;
SELECT Payment_ID, Transaction_ID, Payment_Method, Status FROM Payments;
SELECT Refund_ID, Transaction_ID, Items_Returned, Refund_Amount FROM Refunds_Returns;
